...
const menuItems = [
  ...
  { to: '/user', icon: <FaUserCircle />, label: 'User Profile' },
];
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="md:hidden z-12 relative top-4 left-4">
      ...
      {/* Sidebar */}
      <div
        className={` absolute top-0 left-0 bottom-0 z-40 bg-white p-2 transition-all duration-300 ease-in-out flex flex-col
      ...
      >
        ...
        <div className="flex-col items-center  justify-between mb-8">
          {/* <Link to="/" className="flex items-center">
            <svg width="32" height="34" viewBox="0 0 40 43" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <ul className={`space-y-4 font-bold mt-8`}>
          {menuItems.map((item, idx) => (
            <li key={idx}>
              <Link
                to={item.to}
                className={`flex items-center text-primary hover:text-accent transition-all duration-200
